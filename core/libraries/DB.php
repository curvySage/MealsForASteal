...
//		self::$instance->select_db($db);
	...
	public function query($sql) {
		...
		if (! $result) {
			...
			if (IN_PRODUCTION) {
				...
				# Email app owner
				$subject = "SQL Error";
				$body    = "<h2>SQL Error</h2> ".$sql." ".mysqli_error($this->connection);
			    ...
			    die("<h2>There's been an error processing your request (#DB138)</h2>");
		 		...
		 		die(Debug::dump("SQL Error: ".$sql." ".mysqli_error($this->connection)));
			}
		}		
	...
	public function select_row($sql, $type = 'assoc') {
	    $result = $this->query($sql);
	    $mysql_fetch = 'mysqli_fetch_'.$type;
	...
	public function insert_row($table, $data) {
		...
		foreach ($data as $column => $value)
		    ...
		    $sql .= " $column = '".mysqli_real_escape_string($this->connection, $value)."',";
		...
		# Rows
			...
			foreach($data as $row) {				
				...
				foreach($row as $field => $value) {
				    ...
				    $row_string .= "'".mysqli_real_escape_string($this->connection, $value)."',";
				}	
	...
	public function update_or_insert_row($table, $data) {
		...
		# Build fields and values
			...
			foreach($data as $field => $value) {
				...
				$dup    .= $field."="."'".mysqli_real_escape_string($this->connection, $value)."',";
			}
		...
		if(is_array($data)){
			...
			foreach($data as $k => $v){
				if(is_array($v)){
					...
					$data[$k] = mysqli_real_escape_string($v, $this->connection);
				}
			}
