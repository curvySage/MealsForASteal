  ...
  <div id="content-section">
    <div class="profile">
      ...
      <div class="profile-side">
        <img src="public/img/user.svg" alt="ProfilePic">
      ...
      <div class="posts-side">
        <div class="posting-selection">
          <div>
            <a class="selected" href=" ">Recent</a>
            <a href=" ">Top Upvoted</a>
          </div>
        </div>
        <!-- posts-->
          ...
          if (mysqli_num_rows($posts_array) == 0) {
            ...
            while ($row = mysqli_fetch_assoc($posts_array)) {
              ...
              if ((!isset($vote) || $vote == 0) && isset($_COOKIE['token'])) {
                echo '
                  <img id="rep_up_'.$row['recipe_id'].'" class="upvote-button" src="public/img/voting/upvote-not-selected.svg" alt="upvote">
                  <span class="score">'.$row['sum(f.vote)'].'</span>
                  <img id="rep_down_'.$row['recipe_id'].'" class="downvote-button" src="public/img/voting/downvote-not-selected.svg" alt="downvote">
                ';
              } else 

              if ($vote == 1 && isset($_COOKIE['token'])) {
               echo '
                  <img id="rep_up_'.$row['recipe_id'].'" class="upvote-button" src="public/img/voting/upvote-selected.svg" alt="upvote">
                  <span class="score">'.$row['sum(f.vote)'].'</span>
                  <img id="rep_down_'.$row['recipe_id'].'" class="downvote-button" src="public/img/voting/downvote-not-selected.svg" alt="downvote">
                '; 
              } else 
              ...
              if ($vote == -1 && isset($_COOKIE['token'])) {
                echo '
                  <img id="rep_up_'.$row['recipe_id'].'" class="upvote-button" src="public/img/voting/upvote-not-selected.svg" alt="upvote">
                  <span class="score">'.$row['sum(f.vote)'].'</span>
                  <img id="rep_down_'.$row['recipe_id'].'" class="downvote-button" src="public/img/voting/downvote-selected.svg" alt="downvote">
                ';
              }
